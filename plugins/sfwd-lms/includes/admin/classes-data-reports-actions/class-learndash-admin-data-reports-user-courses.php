...
class Learndash_Admin_Data_Reports_Courses extends Learndash_Admin_Settings_Data_Reports {
	...
	private function initialize_report_data( array $data ): array {
		...
		// Process filters if provided, otherwise use default group-based filtering.
		if (
			isset( $data['filters'] )
			&& ! empty( $data['filters'] )
		) {
			// Use custom filters provided in the request.
			$this->transient_data = wp_parse_args( $this->transient_data, $data );
		} else {
			// Default: filter by group if group_id is provided.
			if (
				isset( $data['group_id'] )
				&& ! empty( $data['group_id'] )
			) {
				// Get all user IDs for the specified group.
				$group_id_value                    = $data['group_id'];
				$group_id                          = intval( (string) $group_id_value );
				$this->transient_data['users_ids'] = learndash_get_groups_user_ids( $group_id );
				// Get all course IDs enrolled in the group.
				$this->transient_data['posts_ids'] = learndash_group_enrolled_courses( $group_id );
			}
			$this->transient_data['activity_status'] = [ 'NOT_STARTED', 'IN_PROGRESS', 'COMPLETED' ];
		}
